...
# 
#
# multistep method test for multijoint case   
#    
#                           - 13        - 16 [lsh] - 18 [lelb] - 20 [lwr] - 22
#     3        - 6          - 9         - 12       - 15
#                           - 14        - 17 [rsh] - 19 [relb] - 21 [rwr] - 23
...
#     2 [rhip] - 5 [rknee] - 8 [rankle] - 11                
###############################################################################
def checkLBSModel_multistep(smpl_model, cam_s, pose_s, cam_t, pose_t, synthetic, Flip, img_s = None, img_t = None):
    ...
    if not Flip:
        if not synthetic:
            ...
            pose[18*3:19*3] =  rodrig_y90[:]  # lelbow, + 90:     
    else:
        pose_tmp = np.zeros(smpl_model.pose.size)
        if not synthetic:
            #pose_tmp[0:3] = pose_s[0:3]
    ...
    if not Flip:
        if not synthetic:
            ...
            pose[20*3:21*3] =  rodrig_x90[:]  # lwrist, + 90 :     
    else:
        pose_tmp = np.zeros(smpl_model.pose.size)
        if not synthetic:
            #pose_tmp[0:3] = pose_s[0:3]
    ...
    plt.subplot(1,numFigures,startFigure + 5)
    plt.imshow(Renderer.r) #.copy()[::-1,:,:])
    plt.title('Target in Rest')
    ...
    plt.subplot(1,numFigures,startFigure + 6)
    plt.imshow(Renderer.r) #.copy()[::-1,:,:])
    plt.title('Target Posed')
...
def posexfer(smpl_model, source,  target, outpath):
    ...
    checkLBSModel_multistep(smpl_model, cam_s, pose_s, cam_t, pose_t, True, False, img_s, img_t)
