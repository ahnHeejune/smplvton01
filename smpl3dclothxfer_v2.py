...
def render_cloth(cam, _texture, texture_v2d, faces, imHuman):
    ...
    overlay_renderer = smpl3dclothrec_v2.build_texture_renderer(cam, cam.v, faces, texture_v2d, faces,
...
def cloth3dxfer_single(smpl_model, src_param_path, tgt_param_path, cloth_path, clothmask_path, human_path, human_segm_path, ocloth_path, omask_path):

    # 1. reconstruct 3D cloth from template
    params_src, body, diff_cloth_body, texture, texture_v2d, face4cloth = smpl3dclothrec_v2.cloth3drec_single(
    ...
    body_tgt_sv = smpl3dclothrec_v2.build_smplbody_surface(
    ...
    imCloth3dWarped, imClothMask3dWarped = render_cloth(
        cam_tgt, texture, texture_v2d, face4cloth, imHuman)
    ...
    """img_white = np.zeros(
        [imCloth3dWarped.shape[0], imCloth3dWarped.shape[1], 3], dtype=np.uint8)
    img_white.fill(255)
    # or img_white[:] = 255
...
if __name__ == '__main__':
    ...
    for i in range(len(test_pairs)):
        ...
        cloth3dxfer_single(smpl_model, template_smpl_param_path, human_smpl_param_path,
                           cloth_path, clothmask_path, human_image_path, human_segm_path, ocloth_path, oclothmask_path)
